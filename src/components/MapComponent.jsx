...
useEffect(() => {
  ...
  if (driverMarkerRef.current) {
    driverMarkerRef.current.setMap(null); // remove old marker if any
  }
...
useEffect(() => {
  ...
  // Get current marker position, or fallback to newPos if null
  ...
  animate();
    ...
    // 3. Get user (map center) location
    useEffect(() => {
    ...
    // *** NEW: Load parcels for the driverUid to know the destination(s) ***
    // We assume selectedDriver.id (uid) is the driver’s identifier
    ...
    useEffect(() => {
      ...
      // Listen to parcels with this driverUid
    ...
    // *** NEW: Compute route from driver → parcel (fastest) whenever driver location or parcels change ***
  useEffect(() => {
    if (
      !isLoaded ||
      !selectedDriver ||
    ...
    directionsService.route(
      {
        ...
        optimizeWaypoints: false, // because we're sorting manually
        drivingOptions: {
    ...
    return (
      <div style={{ position: "relative" }}>
        ...
        >
          ...
          {/* User (map-center) location marker */}
          <Marker
            position={center}
            icon={{
              url: deliverLogo,
          ...
          {existingSlowdowns.map((zone, idx) => {
            ...
            if (addingSlowdown) {
              return null; // Hide existing while adding
            }
            return (
              <Circle
                key={`slowdown-${idx}`}
          ...
          {directions && (
            ...
            {/* Parcel markers starting from 1 */}
          {[...driverParcels]
